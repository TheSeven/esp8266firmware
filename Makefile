...
ESPTOOL := python ../tools/bin/esptool.py

CFLAGS_GENERAL  := -c -mlongcalls -mtext-section-literals -ffunction-sections -fdata-sections -fmessage-length=0 -Wall -Wno-packed-bitfield-compat $(CFLAGS_GENERAL)
...
PPFLAGS_GENERAL := -DTARGET_$(subst /,_,$(TARGET)) -DTARGET=$(TARGET) -Isrc
...
active:
	...
	$(Q)+$(MAKE) $(ACTIVE) COPYTO=build/ACTIVE COPYCTL=src/target/ACTIVE
...
build/$(TARGET)/$(TYPE)/0x00000.bin: build/$(TARGET)/$(TYPE)/$(NAME).elf $(COPYCTL)
	$(VQ)echo "[FW]    " $<
	$(Q)$(ESPTOOL) elf2image $(ESPTOOL_ARGS) $< -o build/$(TARGET)/$(TYPE)/
ifneq ($(COPYTO),)
	$(VQ)rm $(COPYTO)/0x*.bin
	$(VQ)cp build/$(TARGET)/$(TYPE)/0x*.bin $(COPYTO)
endif
